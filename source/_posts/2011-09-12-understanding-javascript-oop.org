...
- Note ::
  ...
  engine. Usually, it would be the same as explicitly applying the
  ...
  #+begin_src js
  window.value = 2
  add.apply(1, 1) // this === window
  // => 3

  void function() {
    "use strict"
    add.apply(1, 1) // this === 1
    // => NaN
...
function Mikhail(age, gender) {
    resend(this, 'constructor', 'Mikhail', 'Weiß')
}
...
~[⁣[Prototype]⁣]~ link for the constructed object — which is what we're
interested about.
...
All properties defined will inevitable have ~writable~, ~configurable~
and ~enumerable~ set to true, which is usually not really that much of a
