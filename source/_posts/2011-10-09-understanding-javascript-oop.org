...
object defining their own behaviour, instead of a common /class/ mean that
...
[[http://es5.github.com/#x7.6][valid JavaScript IdentifierName]][fn:2], and doesn't allow for arbitrary expressions
...
[fn:2]: While an =IdentifierName= also allows reserved words, you should
        be aware that ECMAScript engines that aren't fully compliant
        with the ECMAScript 5 specs *may* choke on reserved words when
...
[fn:3]: It should be noted that, while ECMAScript-defined native objects
...
=[⁣[Prototype]⁣]=. You can't change this slot directly[fn:4], so the only
...
[fn:4] Some engines *do* expose the =[⁣[Prototype]⁣]= slot, usually
...
Thanks to [[https://github.com/hughfdjackson][@hughfdjackson]], Alex and Taylor for the additional revision of
the article.

Thanks to =--= for pointing out (in the comments) that properties expect
an =IdentifierName=, rather than a plain =Identifier=, which would allow
reserved words (like =foo.class= or =foo.null=) to be used unquoted.
