...
#+begin_src haskell
  ...
  -- | A more semantic way of getting a Property than Property.make()
...
** And finally, the transform combinator

Now that we have =repeat= and =asGenerator= and =choice=, and we can combine
them in any way we want to generate rather complex data generators (take a look
at the =data.ls= file in Claire to see this in action!), we need just one more
thing to make it perfect: a =fold= operator.

So, we have =repeat= that can give us many values, but sometimes we want a
generator that takes a list of generated data, and gives us back a single,
consolidated value. Enter the =transform= combinator, which uses a function to
reduce those values:

#+begin_src js
  function join(xs) { return xs.join('') }

  var string = transform(join, repeat(char))
#+end_src

And, again, the simplified implementation is straightforward:

#+begin_src js
  function transform(f, g) {
    g = asGenerator(g)
    return g.derive({ next: function(){ return f(g.next()) } })
  }
#+end_src
